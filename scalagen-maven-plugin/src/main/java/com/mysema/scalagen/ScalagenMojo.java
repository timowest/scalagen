...
@Mojo( name = "main", defaultPhase = LifecyclePhase.GENERATE_SOURCES )
public class ScalagenMojo extends AbstractMojo {

    @Parameter( defaultValue = "${project}", readonly = true, required = true )
    private MavenProject project;

    @Parameter( defaultValue = "src/main/scala" )
    private String targetFolder;

    @Override
    public void execute() throws MojoExecutionException, MojoFailureException {
        final File in = new File(project.getBuild().getSourceDirectory());
        if (in.exists()) {
